  ...
  <Frame>
    ...
    <div style={{padding: '36px 60px', height: 'calc(100% - 40px)', overflow: 'hidden'}}>
      ...
      <div style={{display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginTop: 32}}>
        {[
          {n: 'Starter', d: 'pra times explorando o produto', p: 'R$ 0', sub: '/mês', cta: 'Começar agora', feats: ['até 100 membros', 'feed + cursos básicos', 'gamificação leve', 'comunidade pública'], primary: false},
          {n: 'Professional', d: 'pra times comerciais consolidados', p: 'R$ 79', sub: '/mês · anual', cta: 'Assinar este plano →', feats: ['membros ilimitados', 'cursos com módulos', 'Stripe billing portal', 'gamificação full', 'eventos com RSVP', 'analytics'], primary: true, badge: 'mais popular'},
          {n: 'Enterprise', d: 'pra empresas em escala', p: 'R$ 199', sub: '/mês · anual', cta: 'Falar com vendas', feats: ['tudo do Pro', 'white-label', 'API + webhooks', 'discovery enterprise', 'SLA + suporte', 'sem branding PortalHub'], primary: false},
        ].map((t, i) => (
            ...
            <div style={{marginTop: 22, display: 'flex', flexDirection: 'column', gap: 8}}>
              {t.feats.map((f, j) => (
                <div key={j} style={{display: 'flex', gap: 8, fontSize: 13}}>
                  <span style={{color: 'var(--accent)', flex: '0 0 auto', fontFamily: '"Source Serif 4", "Source Serif Pro", Georgia, serif', fontSize: 18, lineHeight: 1, marginTop: -1}}>✓</span>
                  <span>{f}</span>
                </div>
              ))}
            </div>
      ...
      <div style={{display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 24, marginTop: 24}}>
        ...
        <div>
          <Box style={{padding: 18, background: '#fff7ee', borderColor: 'var(--ink)'}}>
            ...
            <Mono style={{color: 'var(--ink-mid)'}}>economiza R$ 184/mês vs. plano fixo</Mono>
