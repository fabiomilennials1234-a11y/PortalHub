...
const FeedClassic = () => (
  <Frame>
    <AppBar org="polaris/exec-circle" tab={[{label: 'Feed', on: true}, {label: 'Cursos'}, {label: 'Eventos'}, {label: 'Membros'}, {label: 'Rank'}]} />
    <div style={{display: 'grid', gridTemplateColumns: '210px 1fr 280px', height: 'calc(100% - 60px)'}}>
      ...
      <div style={{padding: 16, borderRight: '1.5px solid var(--line)', display: 'flex', flexDirection: 'column', gap: 14}}>
        ...
        <div>
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: 4, marginTop: 8}}>
            {[['Todos', 142, true], ['Anúncios', 8], ['Q&A', 34], ['Cases', 21], ['Discussão', 47], ['Estratégia', 12]].map(([n, c, on]) => (
      ...
      <div style={{padding: 16, overflow: 'hidden'}}>
        ...
        <Box style={{padding: 12, display: 'flex', gap: 10, marginBottom: 14}}>
          <Av>FM</Av>
          <div style={{flex: 1, padding: '8px 0', color: 'var(--ink-low)', fontSize: 14}}>Compartilhe um case, dúvida ou aprendizado…</div>
          <Btn sm>anexo</Btn>
          <Btn sm>áudio</Btn>
        ...
        <Box style={{padding: 12, marginBottom: 10, background: '#fff8e6', borderColor: 'var(--ink)'}}>
          <div style={{display: 'flex', gap: 10, alignItems: 'center'}}>
            ...
            <Hand size={18}>Diretrizes da comunidade · leia antes de postar</Hand>
            <div style={{flex: 1}} />
        ...
        {/* Posts */}
        {[
          {a: 'MR', n: 'Maria Ribeiro', role: 'VP Sales · Tier 12', cat: 'Q&A', t: 'há 2h', title: 'Como vocês fazem discovery em deals de R$ 500k+?', body: 'Tô na dúvida entre coluna org_id em tudo vs schemas separados. Quem já passou pelos dois?', up: 34, comments: 12, react: []},
          {a: 'JS', n: 'João Santos', role: 'Head of GTM · Tier 8', cat: 'Cases', t: 'há 5h', title: 'Refiz o playbook de discovery — feedback?', body: 'V2 com 3 perguntas-âncora pra qualificar valor real. Antes/depois nas imagens.', up: 51, comments: 23, react: [], image: true},
          {a: 'DK', n: 'Davi Klein', role: 'CEO · Tier 15', cat: 'Anúncios', t: 'ontem', title: 'Q3 — programa de certificação entrou em campo', body: 'Pontos, níveis e leaderboard. Quem chegar em TIER 5 essa semana ganha vaga na live.', up: 88, comments: 41, react: []},
        ].map((p, i) => (
          <Box key={i} style={{padding: 14, marginBottom: 10}}>
            ...
            <div style={{display: 'flex', gap: 14, alignItems: 'center', marginTop: 10, fontSize: 12.5, color: 'var(--ink-mid)'}}>
              <span>↑ {p.up}</span>
              <span>{p.comments} resp.</span>
              <span style={{display: 'flex', gap: 4}}>{p.react && p.react.map(r => <span key={r} style={{padding: '1px 7px', border: '1px solid var(--line-soft)', borderRadius: 3, fontSize: 10.5, fontFamily: 'Geist Mono', color: 'var(--ink-mid)', textTransform: 'uppercase', letterSpacing: '0.04em'}}>{r} · 4</span>)}</span>
              <div style={{flex: 1}} />
              <Mono>+8 créditos ao responder</Mono>
            </div>
          </Box>
        ))}
      </div>
      ...
      <div style={{padding: 16, borderLeft: '1.5px dashed var(--line-soft)', display: 'flex', flexDirection: 'column', gap: 14}}>
        <Box style={{padding: 12, background: '#fff7ee', borderColor: 'var(--ink)'}}>
          ...
          <div style={{display: 'flex', alignItems: 'center', gap: 10, marginTop: 8}}>
            <LvlBadge lvl={7} />
            <Mono style={{color: 'var(--ink-mid)'}}>1,240 créditos</Mono>
          </div>
          <Bar pct={62} style={{marginTop: 8}} />
          <Mono style={{color: 'var(--ink-mid)', marginTop: 4, display: 'block'}}>240 créditos pro nível 8</Mono>
        </Box>

        <div>
          <Mono style={{color: 'var(--ink-mid)'}}>EM ALTA HOJE</Mono>
          <div style={{display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8}}>
            {['"Como vocês modelam discovery enterprise"', '"Refiz o playbook de discovery"', '"Pricing — anual vs mensal em B2B"'].map((t, i) => (
        ...
        <div>
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8}}>
            <Box style={{padding: 8, fontSize: 12}}>
              <Mono>QUI · 19h</Mono>
              <div style={{fontWeight: 600, marginTop: 2}}>Roadmap Q3 com a liderança</div>
...
const FeedStream = () => (
  <Frame>
    ...
    <div style={{display: 'flex', alignItems: 'center', padding: '10px 18px', borderBottom: '1.5px solid var(--line)', gap: 12}}>
      <Hand size={20}>acme<span style={{color: 'var(--accent)'}}>/</span>exec-circle</Hand>
      <div style={{flex: 1}} />
      <Input placeholder="busca, ⌘K" style={{width: 340, padding: '5px 12px', fontSize: 12}} />
      <Pill yellow>1,240 créditos</Pill>
      <Av sm>FM</Av>
    </div>
      ...
      <Box rough style={{padding: 14, gridColumn: 'span 2'}}>
        <div style={{display: 'flex', gap: 10, alignItems: 'center'}}>
          ...
          <div style={{flex: 1}}>
            ...
            <Mono style={{color: 'var(--ink-mid)'}}>Head of GTM · Tier 8 · há 5h · Cases</Mono>
          </div>
          <LvlBadge lvl={8} />
        </div>
        ...
        <div style={{display: 'flex', gap: 12, marginTop: 10, alignItems: 'center'}}>
          ...
          <Pill style={{fontSize: 11}}>23 resp.</Pill>
          <Pill style={{fontSize: 11}}>★ 14</Pill>
          <div style={{flex: 1}} />
          <Btn sm primary>Responder · +8 créditos</Btn>
        </div>
      </Box>

      {[
        {a: 'MR', n: 'Maria', t: 'Como vocês modelam discovery enterprise?', cat: ' Q&A', up: 34, c: 12},
        {a: 'DK', n: 'Davi', t: 'Q3 entrou em campo', cat: 'Anúncio', up: 88, c: 41, pinned: true},
        {a: 'BL', n: 'Bia', t: 'Pricing: mensal x anual?', cat: 'Discussão', up: 22, c: 18},
        {a: 'TR', n: 'Tiago', t: 'Achei um bug no checkout', cat: 'Estratégia', up: 6, c: 3},
      ].map((p, i) => (
    ...
    <div style={{display: 'grid', gridTemplateColumns: '64px 1fr 240px', height: 'calc(100% - 60px)'}}>
      ...
      <div style={{padding: '20px 30px', position: 'relative', overflow: 'hidden'}}>
        ...
        <div style={{position: 'relative', paddingLeft: 30}}>
          ...
          {[
            {t: '09:14', a: 'MR', body: 'Maria abriu uma pergunta em #pergunta', tag: 'discovery deals enterprise', up: 34, c: 12, kind: 'post'},
            {t: '10:02', a: '●', body: 'Você ganhou 25 créditos respondendo ao Carlos', kind: 'gam'},
              ...
              <Box style={{padding: 10, marginTop: 4}}>
                ...
                {(e.up !== undefined) && (
                  <div style={{display: 'flex', gap: 10, marginTop: 6, fontSize: 12, color: 'var(--ink-mid)'}}>
                    <span>↑ {e.up}</span><span>{e.c} resp.</span>
                  </div>
                )}
              </Box>
            </div>
      ...
      <div style={{padding: 16, borderLeft: '1.5px dashed var(--line-soft)', display: 'flex', flexDirection: 'column', gap: 12}}>
        ...
        <Mono style={{color: 'var(--ink-mid)', marginTop: 6}}>SEQUÊNCIA</Mono>
        <Box style={{padding: 10}}>
          <Hand size={28}>14 dias consecutivos</Hand>
